	...
	useEffect(() => {
		...
		void (async () => {
			try {
				...
				if (mounted) setError(e instanceof Error ? e.message : "Failed to load customers");
			} finally {
	...
	return (
		<>
			...
			>
				...
				>
					<KpiCard title="Total Customers" value={String(customers.length)} className={cn("w-xs shrink-0")} />
					<KpiCard title="Active" value={String(active)} className={cn("w-xs shrink-0")} />
					<KpiCard title="Residential" value={String(residential)} className={cn("w-xs shrink-0")} />
					<KpiCard title="Commercial" value={String(commercial)} className={cn("w-xs shrink-0")} />
					<KpiCard title="With No-Shows" value={String(withNoShows)} className={cn("w-xs shrink-0")} />
				</FadeEnd>

				{/* Customer List */}
				<div className={cn("mx-2 w-full bg-background-primary rounded-xl border border-background-secondary relative pt-12")}>
					{/* Header */}
					<div className="border-b border-secondary/50 h-12 absolute top-0 inset-x-4 items-center grid grid-cols-5">
						{["Name", "Type", "Phone", "Address", "Status"].map((col) => (
							<p key={col} className="text-sm font-medium text-foreground/60">{col}</p>
						))}
					</div>
					<ul className="w-full divide-y divide-background-secondary/50 px-4 py-3">
						{loading && (
							<li className="text-xs text-text-tertiary py-3 px-4">Loading customers...</li>
						)}
						{!loading && customers.length === 0 && (
							<li className="text-xs text-text-tertiary py-3 px-4">No customers found. Add one using the button below!</li>
						)}
						{customers.map((c) => (
							<li
								key={c.id}
							...
							>
								...
								<p className={cn("text-sm", c.isActive ? "text-success-text" : "text-text-tertiary")}>
