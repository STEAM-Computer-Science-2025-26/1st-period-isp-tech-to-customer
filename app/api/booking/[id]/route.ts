...
export async function PATCH(
	...
	if (action === "convert") {
		...
		return NextResponse.json({ booking: { ...booking, status: "converted", convertedJobId: job.id }, jobId: job.id });
	}

	return NextResponse.json({ error: "Invalid action. Use: confirm, decline, convert" }, { status: 400 });
}