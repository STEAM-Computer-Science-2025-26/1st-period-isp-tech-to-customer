...
describe("Distance Calculator — Edge Cases", () => {
	...
	test("V1: Boundary coordinates", () => {
		...
		expect(areValidCoordinates({ latitude: -90, longitude: -180 })).toBe(true);
		expect(areValidCoordinates({ latitude: 90.0001, longitude: 0 })).toBe(
			false
		);
	});
	...
	test("V2: NaN and Infinity", () => {
		...
		expect(areValidCoordinates({ latitude: Infinity, longitude: 0 })).toBe(
			false
		);
	});

	test("V3: Wrong shapes", () => {
		expect(areValidCoordinates({ lat: 32, lon: -96 } as unknown as Coord)).toBe(
			false
		);
