...
describe("Pricebook", () => {
	...
	test("GET /pricebook?search=tune filters by search", async () => {
		...
		expect(status).toBe(200);
		expect(
			body.items.some((i: any) => i.name.toLowerCase().includes("tune"))
		).toBe(true);
	});
...
describe("Estimates", () => {
	...
	test("GET /estimates?customerId= filters correctly", async () => {
		const { status, body } = await get(
			`/estimates?customerId=${customerId}`,
			token
		);
		expect(status).toBe(200);
		expect(body.estimates.every((e: any) => e.customerId === customerId)).toBe(
			true
		);
	});
...
describe("Stripe", () => {
	...
	test("POST /stripe/payment-intent on already-paid invoice returns 409", async () => {
		...
		await patch(`/invoices/${invoiceId}`, token, {
			status: "paid",
			amountPaid: 999
		});
...
describe("Parts Usage", () => {
	...
	test("DELETE /jobs/:jobId/parts/:usageId restores inventory", async () => {
		const sql = getSql();
		const [before] =
			(await sql`SELECT quantity FROM parts_inventory WHERE id = ${partId}`) as any[];
...
describe("Truck Inventory", () => {
	...
	test("GET /truck-inventory?vehicleId= returns inventory", async () => {
		const { status, body } = await get(
			`/truck-inventory?vehicleId=${vehicleId}`,
			token
		);
		expect(status).toBe(200);
		expect(Array.isArray(body.inventory)).toBe(true);
		expect(body.inventory.some((i: any) => i.vehicleId === vehicleId)).toBe(
			true
		);
	});
	...
	test("GET /truck-inventory?lowStockOnly=true returns only low stock", async () => {
		...
		const { status, body } = await get(
			`/truck-inventory?vehicleId=${vehicleId}&lowStockOnly=true`,
			token
		);
		expect(status).toBe(200);
		expect(body.inventory.every((i: any) => i.quantity <= i.minQuantity)).toBe(
			true
		);
	});
...
describe("Payment Collection Workflow", () => {
	...
	test("GET /jobs/:jobId/payment-summary returns completed job summary", async () => {
		const { status, body } = await get(
			`/jobs/${closedJobId}/payment-summary`,
			token
		);
		expect(status).toBe(200);
