...
describe('scoreAndRankCandidates – edge cases', () => {
  const baseJob = { id: 'job-001', latitude: 40, longitude: -74, requiredSkills: ['hvac'], isEmergency: false };

  beforeEach(() => jest.clearAllMocks());

  test('defaults avgRating to 3 if undefined', async () => {
    mockedGetBatchDriveTimes.mockResolvedValue([{ durationMinutes: 5 }]);
    const techs = [{ id: 't1', currentLocation: { latitude: 40, longitude: -74 }, isAvailable: true, skills: ['hvac'], currentJobCount: 0 }];
  ...
  test('clamps workload score at 0 for heavy load', async () => {
    mockedGetBatchDriveTimes.mockResolvedValue([{ durationMinutes: 5 }]);
    const techs = [{ id: 't2', currentLocation: { latitude: 40, longitude: -74 }, isAvailable: true, skills: ['hvac'], avgRating: 5, currentJobCount: 10 }];
    const result = await scoreAndRankCandidates(techs, baseJob, false);
  ...
  test('skillScore = 0 if missing required skill', async () => {
    mockedGetBatchDriveTimes.mockResolvedValue([{ durationMinutes: 5 }]);
    const techs = [{ id: 't3', currentLocation: { latitude: 40, longitude: -74 }, isAvailable: true, skills: ['plumbing'], avgRating: 5, currentJobCount: 0 }];
    const result = await scoreAndRankCandidates(techs, baseJob, false);
  ...
  test('treats undefined requiredSkills as full match', async () => {
    mockedGetBatchDriveTimes.mockResolvedValue([{ durationMinutes: 5 }]);
    const techs = [{ id: 't4', currentLocation: { latitude: 40, longitude: -74 }, isAvailable: true, skills: [], avgRating: 5, currentJobCount: 0 }];
  ...
  test('availabilityScore = 0 when unavailable', async () => {
    mockedGetBatchDriveTimes.mockResolvedValue([{ durationMinutes: 5 }]);
    const techs = [{ id: 't5', currentLocation: { latitude: 40, longitude: -74 }, isAvailable: false, skills: ['hvac'], avgRating: 5, currentJobCount: 0 }];
    const result = await scoreAndRankCandidates(techs, baseJob, false);
