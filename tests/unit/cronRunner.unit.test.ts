...
describe("processRecurringSchedules", () => {
	...
	test("creates jobs for multiple due schedules", async () => {
		const makeSchedule = (id: string, freq: string) => ({
			id, company_id: "co-1", branch_id: null, customer_id: "cust-1",
			customer_name: "Alice", customer_phone: null,
			title: "Maintenance", description: null, job_type: "maintenance",
			preferred_tech_id: null, duration_minutes: 60,
			next_run_at: "2025-04-01", advance_days: 3, frequency: freq,
			address: "123 Main", city: "Dallas", state: "TX", zip: "75001"
		});
		makeSqlMock([
			[makeSchedule("s1", "monthly"), makeSchedule("s2", "quarterly")],
			[{ id: "job-1" }], [], // schedule 1: INSERT + UPDATE
			[{ id: "job-2" }], []  // schedule 2: INSERT + UPDATE
		]);
		const result = await processRecurringSchedules();
...
describe("processMembershipRenewals", () => {
	...
	test("counts multiple reminders and renewals correctly", async () => {
		makeSqlMock([
			...
			[
				{ id: "agr-a", customer_name: "Alice", tier_name: "Silver", expires_at: "2025-03-15", email: "a@a.com", phone: null },
				{ id: "agr-b", customer_name: "Bob", tier_name: "Gold", expires_at: "2025-03-18", email: null, phone: "+1214" }
			],
			[], // UPDATE agr-a renewal_notified_at
			[], // UPDATE agr-b renewal_notified_at
			// Two expired with auto_renew=true
			[
				{ id: "agr-x", company_id: "co-1", customer_id: "c1", tier_id: "t1", billing_cycle: "annual", price_locked: "200", auto_renew: true, visits_allowed: 1 },
				{ id: "agr-y", company_id: "co-1", customer_id: "c2", tier_id: "t1", billing_cycle: "annual", price_locked: "200", auto_renew: true, visits_allowed: 1 }
			],
			[{ id: "new-x" }], [], // INSERT + billing_trigger for agr-x
			[{ id: "new-y" }], []  // INSERT + billing_trigger for agr-y
		]);
		const result = await processMembershipRenewals();
...
describe("processBillingTriggers", () => {
	...
	test("processes multiple triggers independently", async () => {
		const triggers = [
			{ id: "t1", agreement_id: "a1", company_id: "co-1", customer_id: "c1", price_locked: "100.00", billing_cycle: "monthly" },
			{ id: "t2", agreement_id: "a2", company_id: "co-1", customer_id: "c2", price_locked: "200.00", billing_cycle: "annual" }
		];
		makeSqlMock([
			triggers,            // SELECT
			[{ seq: "1" }],      // nextval t1
...
describe("scheduleReviewRequests", () => {
	...
	test("schedules review for a completed job with phone (sms channel)", async () => {
		const job = {
			id: "job-1", company_id: "co-1", customer_id: "cust-1",
			completed_at: new Date(Date.now() - 60 * 60000).toISOString(),
			phone: "+12145550123", email: null
		};
		makeSqlMock([
	...
	test("schedules review for a completed job with email (email channel)", async () => {
		const job = {
			id: "job-2", company_id: "co-1", customer_id: "cust-1",
			completed_at: new Date(Date.now() - 60 * 60000).toISOString(),
			phone: null, email: "customer@example.com"
		};
		makeSqlMock([[job], []]);
		const result = await scheduleReviewRequests();
		expect(result.scheduled).toBe(1);
	...
	test("schedules reviews for multiple jobs", async () => {
		const jobs = [
			{ id: "j1", company_id: "co-1", customer_id: "c1", completed_at: new Date().toISOString(), phone: "+1214", email: null },
			{ id: "j2", company_id: "co-1", customer_id: "c2", completed_at: new Date().toISOString(), phone: null, email: "b@b.com" }
		];
		makeSqlMock([[...jobs], [], []]);
		const result = await scheduleReviewRequests();
...
describe("dispatchPendingReviewRequests", () => {
	...
	test("marks request as sent successfully", async () => {
		const req = {
			id: "rev-1", customer_id: "cust-1", job_id: "job-1",
			channel: "sms", phone: "+12145550123", email: null,
			first_name: "John", company_name: "Cool Air Co"
		};
		makeSqlMock([
	...
	test("marks request as failed when UPDATE throws", async () => {
		const req = {
			id: "rev-fail", customer_id: "cust-1", job_id: "job-1",
			channel: "email", phone: null, email: "x@x.com",
			first_name: "Jane", company_name: "AC Pro"
		};
		const sqlFn = jest.fn()
	...
	test("handles mixed success and failure across multiple requests", async () => {
		const requests = [
			{ id: "r1", customer_id: "c1", job_id: "j1", channel: "sms", phone: "+1", email: null, first_name: "A", company_name: "Co" },
			{ id: "r2", customer_id: "c2", job_id: "j2", channel: "email", phone: null, email: "b@b.com", first_name: "B", company_name: "Co" }
		];
		const sqlFn = jest.fn()
