...
describe("Stage 2 — Scoring System: Ultimate Bulletproof Tests", () => {
	...
	it("Distance: exactly 0, 50, >50 miles", () => {
		...
		const max = scoreTechnician(
			{ ...baseTech, id: "max", latitude: 32.0, longitude: -96.0 },
			baseJob
		);
		const beyond = scoreTechnician(
			{ ...baseTech, id: "beyond", latitude: 0, longitude: 0 },
			baseJob
		);
	...
	it("Availability: 0, full, over max", () => {
		...
		const over = scoreTechnician(
			{ ...baseTech, currentJobsCount: 10 },
			baseJob
		);
	...
	it("Skills: missing, overqualified, underqualified, mixed", () => {
		const missing = scoreTechnician(
			{ ...baseTech, skillLevel: { hvac_repair: 2 } },
			baseJob
		);
		const over = scoreTechnician(
			{ ...baseTech, skillLevel: { hvac_repair: 4, plumbing: 3, electric: 5 } },
			baseJob
		);
		const under = scoreTechnician(
			{ ...baseTech, skillLevel: { hvac_repair: 1, plumbing: 1, electric: 1 } },
			baseJob
		);
		const mixed = scoreTechnician(
			{ ...baseTech, skillLevel: { hvac_repair: 2, plumbing: 1, electric: 3 } },
			baseJob
		);
	...
	it("Performance: <10 jobs, perfect, mid, poor", () => {
		const newTech = scoreTechnician(
			{ ...baseTech, recentJobCount: 5, recentCompletionRate: 1 },
			baseJob
		);
		const perfect = scoreTechnician(
			{ ...baseTech, recentJobCount: 15, recentCompletionRate: 0.95 },
			baseJob
		);
		const mid = scoreTechnician(
			{ ...baseTech, recentJobCount: 20, recentCompletionRate: 0.85 },
			baseJob
		);
		const poor = scoreTechnician(
			{ ...baseTech, recentJobCount: 12, recentCompletionRate: 0.72 },
			baseJob
		);
	...
	it("Workload: 0, half, max, emergency ignored", () => {
		...
		const emergency = scoreTechnician(
			{ ...baseTech, dailyJobCount: 3 },
			emergencyJob
		);
	...
	it("Stress test: 100 random technicians", () => {
		const techs = Array.from({ length: 100 }).map((_, i) => ({
			...
			skillLevel: {
				hvac_repair: Math.floor(Math.random() * 5),
				plumbing: Math.floor(Math.random() * 5),
				electric: Math.floor(Math.random() * 5)
			}
		}));
