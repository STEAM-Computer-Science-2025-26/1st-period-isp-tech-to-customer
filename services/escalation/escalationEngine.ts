...
function jobMatchesPolicy(
	...
	if (conditions.keywords?.length) {
		const haystack = `${job.description ?? ""} ${jobType}`.toLowerCase();
...
export async function triggerEscalation(jobId: string): Promise<{
	...
	for (const policy of policies) {
		if (
			jobMatchesPolicy(
				job,
				policy.trigger_conditions as EscalationTriggerConditions
			)
		) {
	...
	console.log(
		`[escalation] Triggered for job ${jobId} → event ${event.id} (policy: ${matchedPolicy.name})`
	);
...
export async function advanceEscalations(): Promise<{
	advanced: number;
	timedOut: number;
}> {
	const sql = getSql();
