...
import type { EscalationStep, EscalationTriggerConditions } from "../types/agreementTypes"
...
function jobMatchesPolicy(
	...
	if (conditions.priority?.length) {
		if (!conditions.priority.includes(job.priority ?? "normal")) return false;
	}
...
export async function triggerEscalation(jobId: string): Promise<{
	triggered: boolean;
	eventId?: string;
	reason?: string;
}> {
	...
	if (jobs.length === 0) return { triggered: false, reason: "job not found" };
	const job = jobs[0];
	...
	if (existing.length > 0) {
		return { triggered: false, reason: "escalation already active", eventId: existing[0].id };
	}
	...
	for (const policy of policies) {
		if (jobMatchesPolicy(job, policy.trigger_conditions as EscalationTriggerConditions)) {
...
async function executeEscalationStep(
	...
	try {
		...
		for (const recipient of step.notify) {
			console.log(
				`[escalation] Step ${stepIndex}: notify ${recipient} via ${step.channel} — ${step.message ?? "Emergency job escalation"}`
			);
		}
...
export async function advanceEscalations(): Promise<{ advanced: number; timedOut: number }> {
	const sql = getSql();
	let advanced = 0;
	let timedOut = 0;
	...
	for (const event of activeEvents) {
		const steps: EscalationStep[] = event.steps ?? [];
