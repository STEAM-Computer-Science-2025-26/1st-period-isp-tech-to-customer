...
export class DispatchOrchestrator {
	constructor(
		private techRepo: TechnicianRepository,
	...
	async dispatchJob(
		...
		if (!job.latitude || !job.longitude) {
			throw new Error(
				`Job ${jobId} has no coordinates. Geocoding status: ${job.geocodingStatus}`
			);
		}

		// 3. Load eligible techs
		const techRecords = await this.techRepo.findEligibleForDispatch(
			job.companyId
		);
	...
	private async assignJobToTech(
		...
		try {
			...
			await this.jobRepo.logAssignment(
				jobId,
				techId,
				job.companyId,
				assignedByUserId,
