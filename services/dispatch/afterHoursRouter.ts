...
function isAfterHoursWindow(
	...
	if (isWeekend && weekendAllDay) return true;
...
export async function evaluateAfterHours(
	...
	const isAH = isAfterHoursWindow(
		checkTime,
		rule.weekday_start,
		rule.weekday_end,
		rule.weekend_all_day
	);
 ...
 */
export async function pickOnCallTech(
	onCallEmployeeIds: string[]
): Promise<string | null> {
	if (onCallEmployeeIds.length === 0) return null;
