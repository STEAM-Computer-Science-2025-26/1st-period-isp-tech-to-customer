...
//
// Endpoints:
//   GET    /kpi/thresholds           — list company thresholds
//   POST   /kpi/thresholds           — create threshold
//   PATCH  /kpi/thresholds/:id       — update threshold
//   DELETE /kpi/thresholds/:id       — delete threshold
//   GET    /kpi/alerts               — list fired alerts (unread first)
//   PATCH  /kpi/alerts/:id/read      — mark alert read
//   PATCH  /kpi/alerts/:id/resolve   — mark alert resolved
//   POST   /kpi/check                — manually trigger threshold evaluation
...
const createThresholdSchema = z.object({
	...
	(d) => d.warnBelow != null || d.critBelow != null || d.warnAbove != null || d.critAbove != null,
...
// ─────────────────────────────────────────────────────────────────────────────
// Core KPI evaluation — exported so cron can call it too
// ─────────────────────────────────────────────────────────────────────────────
...
export async function evaluateKpiThresholds(companyId: string): Promise<{
	...
	const metricValues: Record<string, number | null> = {
		first_time_fix_rate: metrics?.first_time_fix_rate != null ? parseFloat(metrics.first_time_fix_rate) : null,
		callback_rate:        metrics?.callback_rate        != null ? parseFloat(metrics.callback_rate)       : null,
		avg_customer_rating:  metrics?.avg_customer_rating  != null ? parseFloat(metrics.avg_customer_rating) : null,
		jobs_completed_30d:   metrics?.jobs_completed_30d   != null ? parseInt(metrics.jobs_completed_30d)    : null
	};
	...
	for (const threshold of thresholds) {
		...
		const checks = [
			{ level: "warning",  breach: threshold.warn_below != null && value < threshold.warn_below },
			{ level: "critical", breach: threshold.crit_below != null && value < threshold.crit_below },
			{ level: "warning",  breach: threshold.warn_above != null && value > threshold.warn_above },
			{ level: "critical", breach: threshold.crit_above != null && value > threshold.crit_above }
		];
		...
		for (const check of checks) {
			...
			const thresholdValue =
				threshold.warn_below ?? threshold.crit_below ??
				threshold.warn_above ?? threshold.crit_above;
...
export async function kpiRoutes(fastify: FastifyInstance) {
	...
	fastify.post(
		...
		async (request, reply) => {
			...
			return reply.code(201).send({ threshold: row });
		}
	);
