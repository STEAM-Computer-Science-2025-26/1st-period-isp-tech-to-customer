...
async function getForecastedJobCount(
	sql: any,
	companyId: string,
	targetMonth: number,
	targetYear: number,
...
export async function preStaffingAlertRoutes(fastify: FastifyInstance) {
		...
		r.get("/staffing-alerts/evaluate", async (request, reply) => {
			...
			for (let i = 1; i <= horizon; i++) {
				...
				for (const rule of rules) {
					...
					const months: number[] = rule.target_months ?? [];
					if (months.length > 0 && !months.includes(targetMonth)) continue;

					// Only fire if we're within the lead window
					if (daysUntil > rule.lead_days) continue;

					const forecastedJobs = await getForecastedJobCount(
						sql,
						companyId!,
						targetMonth,
						targetYear,
