...
export async function certificationRoutes(fastify: FastifyInstance) {
	...
	fastify.get(
		...
		async (request, reply) => {
			const user = getUser(request);
			const { days = "60" } = request.query as { days?: string };
			const companyId = resolveCompanyId(user);
			const sql = getSql();
	...
	fastify.patch(
		...
		async (request, reply) => {
			...
			const result = (await (sql as any)(
				`UPDATE tech_certifications SET ${fullClause} ${where} RETURNING id`,
				whereValues
			)) as CertRow[];
	...
	fastify.post(
		...
		`;
			...
			if ((toAlert as any[]).length === 0) {
				return reply.send({ alerted: 0, certifications: [] });
			}

			// Mark them all as alerted
			const alertIds = (toAlert as any[]).map((c) => c.id);
			await sql`
