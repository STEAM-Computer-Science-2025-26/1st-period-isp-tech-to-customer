...
export async function partsRoutes(fastify: FastifyInstance) {
	...
	fastify.post(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			const techId = body.techId ?? (user as any).userId ?? null;
	...
	fastify.get(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			const totalCost = (parts as any[]).reduce(
				(sum: number, p: any) =>
					sum + Number(p.unitCostAtTime ?? 0) * p.quantityUsed,
				0
			);
	...
	fastify.delete(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			const user = getUser(request);
			const { jobId, usageId } = request.params as {
				jobId: string;
				usageId: string;
			};
			const companyId = resolveCompanyId(user);
	...
	fastify.patch(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			const user = getUser(request);
			const { vehicleId, partId } = request.params as {
				vehicleId: string;
				partId: string;
			};
