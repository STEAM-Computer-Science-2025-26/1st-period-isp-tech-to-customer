...
export async function financingRoutes(fastify: FastifyInstance) {
	...
	fastify.post(
		"/financing/applications/:id/approve",
		{ preHandler: [authenticate] },
		async (request: FastifyRequest, reply: FastifyReply) => {
			const user = getUser(request);
			const { id } = request.params as { id: string };
	...
	fastify.post(
		"/financing/applications/:id/decline",
		{ preHandler: [authenticate] },
		async (request: FastifyRequest, reply: FastifyReply) => {
			const user = getUser(request);
			const { id } = request.params as { id: string };
	...
	fastify.post(
		"/financing/plans/:id/record-payment",
		{ preHandler: [authenticate] },
		async (request: FastifyRequest, reply: FastifyReply) => {
			const user = getUser(request);
			const { id } = request.params as { id: string };
			...
			const newAmountPaid =
				Math.round((Number(plan.amount_paid) + b.amount) * 100) / 100;
			const newBalanceDue = Math.max(
				0,
				Math.round((Number(plan.balance_due) - b.amount) * 100) / 100
			);
			const isFullyPaid = newBalanceDue === 0;

			// Find next payment due date from schedule
			const schedule = plan.payment_schedule ?? [];
			const nextDue = schedule.find((s: any) => s.amount > 0)?.dueDate ?? null;
