...
function requireAdmin(user: JWTPayload, reply: any): boolean {
...
export async function multiRegionRoutes(fastify: FastifyInstance) {
	fastify.register(async (r) => {
		...
		r.put("/regions/:id", async (request, reply) => {
			...
			`) as any[];
			if (!existing) return reply.code(404).send({ error: "Region not found" });
		...
		r.delete("/regions/:id", async (request, reply) => {
			...
			if (!deleted) return reply.code(404).send({ error: "Region not found" });
			return { deleted: true };
		});
		...
		r.post("/regions/:id/branches", async (request, reply) => {
			...
			const { id: regionId } = request.params as { id: string };
			const companyId = resolveCompanyId(user);
			if (!companyId) return reply.code(403).send({ error: "Forbidden" });
		...
		r.delete("/regions/:id/branches/:branchId", async (request, reply) => {
			...
			const { id: regionId, branchId } = request.params as {
				id: string;
				branchId: string;
			};
			const companyId = resolveCompanyId(user);
		...
		r.get("/regions/:id/analytics", async (request, reply) => {
			...
			const branchIds: string[] = (region.branch_ids ?? []).filter(Boolean);
			...
			const techMap = Object.fromEntries(
				techCounts.map((t: any) => [t.branchId, t])
			);

			const enrichedBranches = branchStats.map((b: any) => ({
				...b,
			...
			// Region totals
			const totals = enrichedBranches.reduce(
				(acc: any, b: any) => ({
					...
					revenueCollected:
						Number(acc.revenueCollected) + Number(b.revenueCollected),
			...
			for (const row of capacity) {
				...
				byRegion[row.regionId].branches.push({
					...
					utilizationPct:
						row.totalTechs > 0
							? Math.round(
									((row.totalTechs - row.availableTechs) / row.totalTechs) * 100
								)
							: 0
				});
