...
function resolveCompanyId(user: JWTPayload, body?: string): string | null {
	if (user.role === "dev") return body ?? user.companyId ?? null;
	return user.companyId ?? null;
}

// ─── Routes ──────────────────────────────────────────────────────────────────

export async function competitorPricingRoutes(fastify: FastifyInstance) {
