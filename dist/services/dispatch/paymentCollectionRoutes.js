...
export async function paymentCollectionRoutes(fastify) {
    ...
    fastify.post("/jobs/:jobId/close", { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const intent = await stripe.paymentIntents.create({
            amount: amountCents,
            currency: "usd",
            payment_method_types: body.paymentMethod === "card_present" ? ["card_present"] : ["card"],
            capture_method: body.paymentMethod === "card_present" ? "manual" : "automatic",
            metadata: {
                invoiceId: invoiceId,
                jobId,
        ...
        await sql `
			...
			`;
        return reply.code(200).send({
            message: body.paymentMethod === "card_present"
