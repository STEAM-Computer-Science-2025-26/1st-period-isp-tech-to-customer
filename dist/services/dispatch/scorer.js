...
export async function scoreAndRankCandidates(eligibleTechs, job, isEmergency) {
	...
	const scored = validTechs.map((tech, index) => {
		...
		const safeMinutes =
			Number.isFinite(driveTime?.durationMinutes) &&
		...
		const matchedSkills =
			tech.skills?.filter((s) => requiredSkills.includes(s)).length ?? 0;
		const skillScore =
		...
		// Performance
		const safeRating =
			Number.isFinite(tech.avgRating) && tech.avgRating > 0
				? tech.avgRating
				: 3;
		const performanceScore = (safeRating / 5) * 10;
		// Workload
		const jobCount =
			Number.isFinite(tech.currentJobCount) && tech.currentJobCount > 0
