...
export async function scoreAndRankCandidates(eligibleTechs, job, isEmergency) {
    ...
    const validTechs = eligibleTechs.filter((tech) => tech.currentLocation?.latitude != null &&
    ...
    const scored = validTechs.map((tech, index) => {
        const driveTime = driveTimes[index];
        const safeMinutes = Number.isFinite(driveTime?.durationMinutes) &&
            driveTime.durationMinutes >= 0
        ...
        // Performance
        const safeRating = Number.isFinite(tech.avgRating) && tech.avgRating > 0
            ? tech.avgRating
            : 3;
        const performanceScore = (safeRating / 5) * 10;
        // Workload
        const jobCount = Number.isFinite(tech.currentJobCount) && tech.currentJobCount > 0
