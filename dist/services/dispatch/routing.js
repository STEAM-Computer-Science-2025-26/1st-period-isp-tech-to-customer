...
export async function getDriveTime(fromLat, fromLng, toLat, toLng) {
    ...
    try {
        ...
        const straightLineKm = calculateDistance({ latitude: fromLat, longitude: fromLng }, { latitude: toLat, longitude: toLng });
        return {
    ...
    try {
        ...
        return durations.map((durationSeconds, index) => {
            if (durationSeconds === null) {
                ...
                const straightLineKm = calculateDistance({ latitude: origin.lat, longitude: origin.lng }, {
        ...
        return Promise.all(destinations.map((dest) => getDriveTime(origin.lat, origin.lng, dest.lat, dest.lng)));
    }
}
