...
// Helpers
// ============================================================
async function sendViaTwilio(to, body, fromPhone, accountSid, authToken) {
...
// Route handlers
// ============================================================
export function sendSms(fastify) {
    fastify.post("/sms/send", async (request, reply) => {
        ...
        if (!creds?.accountSid) {
            return reply
                .code(422)
                .send({
                error: "Twilio not configured for this company. Add credentials in settings."
            });
        }
...
export function inboundSmsWebhook(fastify) {
...
export function listSmsMessages(fastify) {
