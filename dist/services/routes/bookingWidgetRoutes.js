...
// ─── Routes ──────────────────────────────────────────────────────────────────
export async function bookingWidgetRoutes(fastify) {
    ...
    fastify.get("/booking/:companySlug/config", async (request, reply) => {
        const { companySlug } = request.params;
        const sql = getSql();
        ...
        return reply.send({
            ...
            confirmationMessage: company.booking_confirmation_message ??
    ...
    fastify.get("/booking/:companySlug/slots", async (request, reply) => {
        const { companySlug } = request.params;
    ...
    fastify.post("/booking/:companySlug/submit", async (request, reply) => {
        const { companySlug } = request.params;
        // Rate limit by IP
        ...
        return reply.code(201).send({
            success: true,
            bookingId: lead.id,
            message: company.booking_confirmation_message ??
                "Thanks! We'll call you within 1 business day to confirm your appointment."
        });
    });
    ...
    fastify.patch("/booking/config", async (request, reply) => {
