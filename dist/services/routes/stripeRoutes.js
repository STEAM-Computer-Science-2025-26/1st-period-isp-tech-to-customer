...
export async function stripeRoutes(fastify) {
	...
	fastify.post(
		...
		async (request, reply) => {
			...
			const intent = await stripe.paymentIntents.create({
				...
				capture_method:
					paymentMethodType === "card_present" ? "manual" : "automatic",
				metadata: {
			...
			try {
				switch (event.type) {
					...
					case "payment_intent.succeeded": {
						...
						`;
						fastify.log.info(
							`Invoice ${invoiceId} marked paid via Stripe intent ${intent.id}`
						);
						break;
					}
					// -------------------------------------------------------
					...
					case "payment_intent.payment_failed": {
						...
						if (intent.metadata?.type === "technician_tip") {
							...
							`;
							fastify.log.warn(
								`Tip failed for intent ${intent.id}: ${intent.last_payment_error?.message ?? "unknown"}`
							);
							break;
						}
						...
						const invoiceId = intent.metadata?.invoiceId;
						fastify.log.warn(
							`Payment failed for invoice ${invoiceId ?? "unknown"}: ${intent.last_payment_error?.message ?? "unknown error"}`
						);
						break;
					}
					// -------------------------------------------------------
					...
					case "charge.refunded": {
						...
						`;
						fastify.log.info(
							`Invoice ${invoice.id} updated after refund. New amount paid: ${newAmountPaid}`
						);
						break;
					}
					default:
	...
	fastify.post(
		...
		async (request, reply) => {
			...
			const newAmountPaid = Math.max(
				0,
				Math.round((Number(invoice.amount_paid) - refundedAmount) * 100) / 100
			);
			const newStatus =
				newAmountPaid <= 0
					? "sent"
	...
	// ----------------------------------------------------------
	fastify.get(
		"/stripe/payment-status/:invoiceId",
		{ preHandler: [authenticate] },
		async (request, reply) => {
			const user = getUser(request);
