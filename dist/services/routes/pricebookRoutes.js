...
// Routes
// ============================================================
export async function pricebookRoutes(fastify) {
    // ----------------------------------------------------------
    // POST /pricebook
    // Create a new pricebook item (labor task, part, or bundle).
    // ----------------------------------------------------------
    fastify.post("/pricebook", { preHandler: [authenticate] }, async (request, reply) => {
    ...
    // ----------------------------------------------------------
    fastify.get("/pricebook", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        ...
        const { itemType, category, isActive, search, limit, offset } = parsed.data;
        const companyId = resolveCompanyId(user);
        const sql = getSql();
        if (!companyId && !isDev(user)) {
            return reply.code(403).send({ error: "Forbidden" });
    ...
    // Single item detail.
    // ----------------------------------------------------------
    fastify.get("/pricebook/:itemId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    // ----------------------------------------------------------
    fastify.patch("/pricebook/:itemId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { itemId } = request.params;
    ...
    // ----------------------------------------------------------
    fastify.delete("/pricebook/:itemId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { itemId } = request.params;
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // ----------------------------------------------------------
    fastify.get("/pricebook/categories", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
