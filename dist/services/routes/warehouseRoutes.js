...
async function logInventoryMovement(sql, opts) {
    await sql `
...
// ─── Routes ──────────────────────────────────────────────────────────────────
export async function warehouseRoutes(fastify) {
    fastify.register(async (r) => {
        ...
        r.get("/warehouse/reorder-queue", async (request, reply) => {
			...
			`);
            const totalEstimatedCost = parts.reduce((sum, p) => sum + Number(p.estimatedCost ?? 0), 0);
            return {
                count: parts.length,
