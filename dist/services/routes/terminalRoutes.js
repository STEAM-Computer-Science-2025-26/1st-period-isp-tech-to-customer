...
export async function terminalRoutes(fastify) {
    ...
    fastify.post("/terminal/connection-token", { preHandler: [authenticate] }, async (request, reply) => {
    ...
    fastify.post("/terminal/readers", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user)) {
            return reply.code(403).send({ error: "Forbidden" });
    ...
    fastify.get("/terminal/readers", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user)) {
            return reply.code(403).send({ error: "Forbidden" });
        ...
        const stripe = getStripe();
        const enriched = await Promise.allSettled(rows.map(async (r) => {
            try {
                const live = await stripe.terminal.readers.retrieve(r.stripe_reader_id);
    ...
    fastify.delete("/terminal/readers/:readerId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    fastify.post("/terminal/readers/:readerId/process-payment", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { readerId } = request.params;
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user)) {
    ...
    fastify.post("/terminal/readers/:readerId/capture", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { readerId } = request.params;
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user)) {
        ...
        // Mark invoice paid
        const amountCaptured = Math.round((intent.amount_received / 100) * 100) / 100;
        const newAmountPaid = Math.min(Number(invoice.total), Math.round((Number(invoice.amount_paid) + amountCaptured) * 100) / 100);
    ...
    fastify.post("/terminal/readers/:readerId/cancel", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { readerId } = request.params;
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user)) {
