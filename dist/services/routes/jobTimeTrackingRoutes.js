...
// Routes
// ─────────────────────────────────────────────────────────────────────────────
export async function jobTimeTrackingRoutes(fastify) {
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    fastify.post("/jobs/:jobId/time-tracking", { preHandler: [authenticate] }, async (request, reply) => {
    ...
    // -------------------------------------------------------------------------
    fastify.patch("/jobs/:jobId/time-tracking/departed", { preHandler: [authenticate] }, async (request, reply) => {
        const { jobId } = request.params;
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // -------------------------------------------------------------------------
    fastify.patch("/jobs/:jobId/time-tracking/arrived", { preHandler: [authenticate] }, async (request, reply) => {
        const { jobId } = request.params;
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // Wrench time begins.
    // -------------------------------------------------------------------------
    fastify.patch("/jobs/:jobId/time-tracking/work-started", { preHandler: [authenticate] }, async (request, reply) => {
        const { jobId } = request.params;
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // Wrench time ends.
    // -------------------------------------------------------------------------
    fastify.patch("/jobs/:jobId/time-tracking/work-ended", { preHandler: [authenticate] }, async (request, reply) => {
        const { jobId } = request.params;
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // -------------------------------------------------------------------------
    fastify.patch("/jobs/:jobId/time-tracking/departed-job", { preHandler: [authenticate] }, async (request, reply) => {
        const { jobId } = request.params;
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
        ...
        const driveMinutes = row.departed_at && row.arrived_at
            ? Math.round((new Date(row.arrived_at).getTime() -
                new Date(row.departed_at).getTime()) /
                60000)
            : null;
        const wrenchMinutes = row.work_started_at && row.work_ended_at
            ? Math.round((new Date(row.work_ended_at).getTime() -
                new Date(row.work_started_at).getTime()) /
                60000)
            : null;
        const actualMinutes = row.arrived_at && row.departed_job_at
            ? Math.round((new Date(row.departed_job_at).getTime() -
    ...
    // -------------------------------------------------------------------------
    fastify.get("/jobs/:jobId/time-tracking", { preHandler: [authenticate] }, async (request, reply) => {
        const { jobId } = request.params;
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
