...
function normalizeDbParam(value) {
...
function normalizeColumnType(input) {
    ...
    if (simple.includes(upper))
        return upper;
    const varchar = upper.match(/^VARCHAR\((\d{1,5})\)$/);
    if (varchar)
        return `VARCHAR(${varchar[1]})`;
    const dec = upper.match(/^(DECIMAL|NUMERIC)\((\d{1,3}),(\d{1,3})\)$/);
    if (dec)
        return `${dec[1]}(${dec[2]},${dec[3]})`;
    return null;
}
function normalizeDefaultExpression(input) {
    ...
    if (/^NOW\(\)$/i.test(raw))
        return "NOW()";
    if (/^GEN_RANDOM_UUID\(\)$/i.test(raw))
        return "gen_random_uuid()";
    return `'${raw.replaceAll("'", "''")}'`;
}
async function execRawSql(sqlClient, text, params) {
...
export async function devRoutes(fastify) {
    ...
    fastify.get("/dev/db/table", async (request, reply) => {
        ...
        if (!name)
            return reply.code(400).send({ error: "Missing table name" });
		...
		`;
        const rows = await execRawSql(sql, `SELECT * FROM ${quoteIdent(name)} LIMIT 200`);
    ...
    fastify.post("/dev/db/query", async (request, reply) => {
        if (!devDbEnabled())
            return disabled(reply);
        const sqlText = request.body?.sql;
        if (typeof sqlText !== "string")
            return reply.code(400).send({ error: "Missing sql" });
        if (!isSafeReadOnlySql(sqlText)) {
            return reply.code(400).send({
                error: "Only single-statement read-only SQL allowed (SELECT/WITH/EXPLAIN/SHOW)."
            });
        }
        const sql = getSql();
        const rows = await execRawSql(sql, sqlText.trim());
    ...
    fastify.post("/dev/db/add-column", async (request, reply) => {
        ...
        const defaultExpr = typeof defaultValue === "string" && defaultValue.trim()
    ...
    fastify.post("/dev/db/alter-column", async (request, reply) => {
        ...
        const effectiveName = typeof newName === "string" &&
            newName.trim() &&
    ...
    fastify.post("/dev/db/add-row", async (request, reply) => {
        ...
        const rows = await execRawSql(sql, text, params);
