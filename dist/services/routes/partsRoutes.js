...
export async function partsRoutes(fastify) {
    ...
    // ----------------------------------------------------------
    fastify.post("/jobs/:jobId/parts", { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const [updatedPart] = (await sql `
			...
			`);
        const needsReorder = updatedPart.reorder_level !== null &&
    ...
    // ----------------------------------------------------------
    fastify.get("/jobs/:jobId/parts", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { jobId } = request.params;
        const companyId = resolveCompanyId(user);
        const sql = getSql();
        ...
        const parts = await sql `
			...
			`;
        const totalCost = parts.reduce((sum, p) => sum + Number(p.unitCostAtTime ?? 0) * p.quantityUsed, 0);
        return reply.send({
            partsUsed: parts,
    ...
    // ----------------------------------------------------------
    fastify.get("/parts/usage", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    // ----------------------------------------------------------
    fastify.delete("/jobs/:jobId/parts/:usageId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    // ----------------------------------------------------------
    fastify.put("/truck-inventory", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    // ----------------------------------------------------------
    fastify.patch("/truck-inventory/:vehicleId/:partId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        ...
        if (!item) {
            return reply.code(404).send({
                error: "Truck inventory record not found. Use PUT /truck-inventory to create it."
            });
        }
    ...
    // ----------------------------------------------------------
    fastify.get("/truck-inventory", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    // ----------------------------------------------------------
    fastify.get("/truck-inventory/vehicles", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // ----------------------------------------------------------
    fastify.delete("/truck-inventory/:vehicleId/:partId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { vehicleId, partId } = request.params;
        const companyId = resolveCompanyId(user);
        const sql = getSql();
