 ...
 */
async function geocodeJobAsync(jobId, address) {
    try {
        const geo = await tryGeocodeJob(address);
        await runQuery(`UPDATE jobs
        ...
        await runQuery(`UPDATE jobs SET geocoding_status = 'failed', updated_at = NOW() WHERE id = $1`, [jobId]).catch((err) => console.error("Failed to update geocoding status:", err));
    }
}
...
// Route Handlers
// ============================================================
export function listJobs(fastify) {
...
export function createJob(fastify) {
    fastify.post("/jobs", async (request, reply) => {
        ...
        const result = (await runQuery(`INSERT INTO jobs (
...
export function getJob(fastify) {
    fastify.get("/jobs/:jobId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getAuthUser(request);
        const dev = isDev(user);
        const companyId = requireCompanyId(user);
        const { jobId } = request.params;
        const result = await runQuery(`SELECT ${JOB_SELECT} FROM jobs
			 WHERE id = $1
			   AND ($2::boolean OR company_id = $3)`, [jobId, dev && !companyId, companyId]);
        if (!result[0])
            return reply.code(404).send({ error: "Job not found" });
        return reply.send({ job: result[0] });
    });
}
export function updateJobStatus(fastify) {
    fastify.put("/jobs/:jobId/status", async (request, reply) => {
        ...
        const values = [status, completionNotes ?? null];
        values.push(jobId);
        ...
        const result = (await runQuery(`UPDATE jobs
...
export function updateJob(fastify) {
    fastify.patch("/jobs/:jobId", async (request, reply) => {
        ...
        const result = (await runQuery(`UPDATE jobs SET ${updates.join(", ")}, updated_at = NOW() ${where} RETURNING id`, values));
        if (!result[0]) {
            return reply.code(404).send({ error: "Job not found" });
        }
...
export function deleteJob(fastify) {
    fastify.delete("/jobs/:jobId", async (request, reply) => {
        ...
        const result = dev
            ...
            : await runQuery("DELETE FROM jobs WHERE id = $1 AND company_id = $2 RETURNING id", [jobId, companyId]);
        if (!result[0]) {
            return reply.code(404).send({ error: "Job not found" });
        }
...
export function retryGeocoding(fastify) {
    fastify.post("/jobs/:jobId/retry-geocoding", async (request, reply) => {
        ...
        const result = await runQuery(`SELECT id, address, geocoding_status AS "geocodingStatus" FROM jobs ${whereClause}`, params);
        if (!result[0]) {
            return reply.code(404).send({ error: "Job not found" });
        }
        const job = result[0];
