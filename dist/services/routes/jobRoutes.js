...
async function geocodeJobAsync(jobId, address) {
    try {
        ...
        await query(`UPDATE jobs SET geocoding_status = 'failed', updated_at = NOW() WHERE id = $1`, [jobId]).catch((err) => console.error("Failed to update geocoding status:", err));
    }
}
...
export function createJob(fastify) {
    fastify.post("/jobs", async (request, reply) => {
        ...
        const result = await query(`INSERT INTO jobs (
...
export function getJob(fastify) {
    fastify.get("/jobs/:jobId", { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const result = await query(`SELECT ${JOB_SELECT} FROM jobs
			 WHERE id = $1
...
export function updateJobStatus(fastify) {
    fastify.put("/jobs/:jobId/status", async (request, reply) => {
        ...
        const result = await query(`UPDATE jobs
...
export function updateJob(fastify) {
    fastify.patch("/jobs/:jobId", async (request, reply) => {
        ...
        const result = await query(`UPDATE jobs SET ${updates.join(", ")}, updated_at = NOW() ${where} RETURNING id`, values);
        if (!result[0]) {
            return reply.code(404).send({ error: "Job not found" });
        }
...
export function deleteJob(fastify) {
    fastify.delete("/jobs/:jobId", async (request, reply) => {
        ...
        const result = dev
            ...
            : await query("DELETE FROM jobs WHERE id = $1 AND company_id = $2 RETURNING id", [jobId, companyId]);
        if (!result[0]) {
            return reply.code(404).send({ error: "Job not found" });
        }
...
export function retryGeocoding(fastify) {
    fastify.post("/jobs/:jobId/retry-geocoding", async (request, reply) => {
        ...
        const result = await query(`SELECT id, address, geocoding_status AS "geocodingStatus" FROM jobs ${whereClause}`, params);
        if (!result[0]) {
            return reply.code(404).send({ error: "Job not found" });
        }
        const job = result[0];
