...
function getQBClient(accessToken, realmId) {
    ...
    return new QuickBooks(process.env.QB_CLIENT_ID, process.env.QB_CLIENT_SECRET, accessToken, false, // no token secret (OAuth2)
    realmId, isSandbox, // use sandbox
...
async function syncCustomerToQB(qb, customer) {
    return new Promise((resolve, reject) => {
        ...
        if (customer.qb_customer_id) {
            ...
            qb.updateCustomer({ ...payload, Id: customer.qb_customer_id, sparse: true }, (err, result) => {
                if (err)
                    return reject(err);
                resolve(result.Id);
            });
...
async function syncInvoiceToQB(qb, invoice, lineItems, qbCustomerId) {
    return new Promise((resolve, reject) => {
        const qbLineItems = lineItems.map((li, index) => ({
        ...
        if (invoice.qb_invoice_id) {
            qb.updateInvoice({ ...payload, Id: invoice.qb_invoice_id, sparse: true }, (err, result) => {
                if (err)
                    return reject(err);
                resolve(result.Id);
            });
            ...
            qb.createInvoice(payload, (err, result) => {
                if (err)
                    return reject(err);
                resolve(result.Id);
            });
...
// Routes
// ============================================================
export async function qbRoutes(fastify) {
    // ----------------------------------------------------------
    // GET /qb/connect
    // Step 1: Redirect user to Intuit auth page.
    // ----------------------------------------------------------
    fastify.get("/qb/connect", { preHandler: [authenticate] }, async (request, reply) => {
        const oauthClient = getOAuthClient();
    ...
    // ----------------------------------------------------------
    fastify.get("/qb/callback", { preHandler: [authenticate] }, async (request, reply) => {
    ...
    // ----------------------------------------------------------
    fastify.get("/qb/status", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
        const [tokenRow] = (await sql `
        ...
        const refreshExpired = new Date(tokenRow.refresh_token_expires_at) < new Date();
        return reply.send({
    ...
    // ----------------------------------------------------------
    fastify.delete("/qb/disconnect", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
    ...
    // ----------------------------------------------------------
    fastify.post("/qb/sync/customer/:customerId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
    ...
    // ----------------------------------------------------------
    fastify.post("/qb/sync/invoice/:invoiceId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        ...
        try {
            ...
            const qbInvoiceId = await syncInvoiceToQB(qb, invoice, lineItems, qbCustomerId);
            await sql `
					UPDATE invoices
    ...
    // ----------------------------------------------------------
    fastify.post("/qb/sync/all", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        const sql = getSql();
        const tokenData = await getValidToken(companyId);
        ...
        for (const invoice of invoices) {
            try {
                ...
                const [freshCustomer] = (await sql `
					...
					`);
                const qbCustomerId = freshCustomer?.qb_customer_id ?? invoice.qb_customer_id;
                if (!qbCustomerId) {
