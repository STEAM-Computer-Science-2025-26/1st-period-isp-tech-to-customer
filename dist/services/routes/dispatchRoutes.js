...
import { runDispatchForJob, getDispatchRecommendations, manualAssignJob } from "../dispatch/dispatchService";
...
export function completeJobRoute(fastify) {
    fastify.post("/jobs/:jobId/complete", async (request, reply) => {
        ...
        const { completionNotes, durationMinutes, firstTimeFix, customerRating } = parsed.data;
        try {
            await completeJob(jobId, completionNotes, durationMinutes, firstTimeFix, customerRating);
            return { success: true };
        }
        catch (error) {
            if (error instanceof Error) {
