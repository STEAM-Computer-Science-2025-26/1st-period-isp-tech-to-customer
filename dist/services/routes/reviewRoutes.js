...
function buildSmsBody(template, customerName, reviewUrl) {
    return template.replace("{name}", customerName).replace("{url}", reviewUrl);
}
const DEFAULT_SMS_TEMPLATE = "Hi {name}! Thanks for choosing us. If you have a moment, we'd love a Google review: {url}";
// ============================================================
// Route handlers
// ============================================================
export function triggerReviewRequest(fastify) {
    fastify.post("/reviews/request", async (request, reply) => {
        ...
        if (delayMinutes === 0) {
            if ((channel === "sms" || channel === "both") &&
                job.phone &&
                settings.accountSid) {
                const smsBody = buildSmsBody(settings.smsTemplate ?? DEFAULT_SMS_TEMPLATE, customerName, reviewUrl);
                try {
...
export function listReviewRequests(fastify) {
 ...
 */
export function trackReviewClick(fastify) {
...
export function updateReviewSettings(fastify) {
