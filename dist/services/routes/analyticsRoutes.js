...
export async function analyticsRoutes(fastify) {
        ...
        const rows = (await sql `
				SELECT
					CASE
						...
						WHEN ${period} = 'month' THEN DATE_TRUNC('month', i.created_at)
						ELSE                          DATE_TRUNC('day',   i.created_at)
					END                                                 AS period_start,
    ...
    // Query params: ?days=30
    // -------------------------------------------------------------------------
    fastify.get("/analytics/dashboard", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user)) {
            return reply.code(403).send({ error: "Forbidden" });
        ...
        const jobsChangePct = jobsYesterday > 0
            ? Math.round(((jobsToday - jobsYesterday) / jobsYesterday) * 1000) /
                100
            : null;
        const avgResponseMinutes = responseCurrent?.avg_response_minutes != null
            ? Number(responseCurrent.avg_response_minutes)
            : null;
        const avgResponsePrev = responsePrevious?.avg_response_minutes != null
            ? Number(responsePrevious.avg_response_minutes)
            : null;
        const avgResponseDeltaMinutes = avgResponseMinutes != null && avgResponsePrev != null
