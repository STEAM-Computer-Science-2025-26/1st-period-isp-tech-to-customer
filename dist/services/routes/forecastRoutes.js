...
// ─── Routes ───────────────────────────────────────────────────────────────────
export async function forecastRoutes(fastify) {
    fastify.register(async (r) => {
        ...
        r.get("/forecast/demand", async (request, reply) => {
            ...
            const allCounts = historical.map((r) => Number(r.job_count));
            const overallAvg = allCounts.length
                ? allCounts.reduce((a, b) => a + b, 0) /
                    allCounts.length
            ...
            if (granularity === "month") {
                ...
                for (let i = 0; i < horizon * 4; i++) {
                    ...
                    const weekOfYear = Math.ceil((d.getDate() +
        ...
        r.get("/forecast/seasonal-trends", async (request, reply) => {
            ...
            // Find peak months
            const allMonthly = rows.map((r) => ({
                month: Number(r.month),
                count: Number(r.job_count)
            }));
            const peakMonth = allMonthly.length
                ? allMonthly.reduce((best, cur) => (cur.count > best.count ? cur : best), allMonthly[0])
                : null;
        ...
        r.get("/forecast/staffing", async (request, reply) => {
            ...
            const overallAvg = historical.length
                ? historical.reduce((s, r) => s + Number(r.job_count), 0) /
                    historical.length
                : 10;
                ...
                staffing.push({
                    ...
                    recommendation: delta > 0
                        ...
                        : delta < 0
                            ...
                            : "Staffing looks good",
                    utilizationPct: Math.round((predictedJobs /
        ...
        r.get("/forecast/parts-demand", async (request, reply) => {
            ...
            const predictions = Object.values(partMap)
                .map((part) => {
                const counts = Object.values(part.monthlyData);
                ...
                .sort((a, b) => b.totalPredictedUnits - a.totalPredictedUnits);
            return {
                horizon,
                parts: predictions,
