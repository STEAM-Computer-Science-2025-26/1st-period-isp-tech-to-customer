...
// Routes
// ─────────────────────────────────────────────────────────────────────────────
export async function refrigerantLogRoutes(fastify) {
    // -------------------------------------------------------------------------
    // POST /refrigerant-logs
    // -------------------------------------------------------------------------
    fastify.post("/refrigerant-logs", { preHandler: [authenticate] }, async (request, reply) => {
        ...
        const serviceDate = body.serviceDate ?? body.loggedAt?.split("T")[0] ?? todayISO();
        const [log] = (await sql `
    ...
    // -------------------------------------------------------------------------
    fastify.get("/refrigerant-logs", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user))
            return reply.code(403).send({ error: "Forbidden" });
        ...
        const conditions = [`rl.company_id = $1`];
        const filterParams = [companyId];
        const push = (val) => filterParams.push(val);
        if (q.jobId) {
    ...
    // -------------------------------------------------------------------------
    fastify.get("/refrigerant-logs/summary", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const companyId = resolveCompanyId(user);
        if (!companyId && !isDev(user))
            return reply.code(403).send({ error: "Forbidden" });
    ...
    // -------------------------------------------------------------------------
    fastify.get("/refrigerant-logs/:logId", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { logId } = request.params;
        const companyId = resolveCompanyId(user);
        // Guard against empty logId (e.g. GET /refrigerant-logs/ with trailing slash)
        if (!logId)
            return reply.code(400).send({ error: "logId is required" });
        const sql = getSql();
        const [log] = (await sql `
    ...
    // -------------------------------------------------------------------------
    fastify.post("/refrigerant-logs/:logId/amend", { preHandler: [authenticate] }, async (request, reply) => {
        const user = getUser(request);
        const { logId } = request.params;
        const companyId = resolveCompanyId(user);
        if (!logId)
