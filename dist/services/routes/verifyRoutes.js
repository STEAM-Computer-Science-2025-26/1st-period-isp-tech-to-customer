...
import { createMagicToken, hashMagicToken, createVerificationSessionToken, hashVerificationSessionToken, encryptVerificationCode, decryptVerificationCode } from "../verifyCrypto";
// ─── Helpers ──────────────────────────────────────────────────────────────────
...
function getPublicOrigin(request) {
    ...
    const proto = getHeaderValue(request.headers, "x-forwarded-proto") ??
        request.protocol ??
        "http";
...
export async function verifyRoutes(fastify) {
    ...
    fastify.post("/verify/send", async (request, reply) => {
        const { email: rawEmail, mode: rawMode } = request.body;
        ...
        const mode = rawMode === "code" || rawMode === "link" ? rawMode : "link";
        const sql = getSql();
        ...
        const emailLimit = await enforceRateLimit(sql, `verify:send:email:${email}`, 3, 60 * 30);
    ...
    fastify.post("/verify/code", async (request, reply) => {
        ...
        const row = rows[0];
        if (!row)
            return reply.code(404).send({ message: "Invalid token" });
    ...
    fastify.post("/verify/status", async (request, reply) => {
        ...
        const row = rows[0];
        if (!row)
            return reply.code(404).send({ message: "Invalid verification" });
        if (!row.session_hash || row.session_hash !== sessionHash) {
            return reply.code(403).send({ message: "Invalid verification session" });
        }
