...
export class GeocodingWorker {
    ...
    async fetchPendingJobs() {
        try {
            let raw;
            if (this.hasRetriesColumn) {
                raw = await query(`SELECT id, address, COALESCE(geocoding_retries, 0) AS geocoding_retries
FROM jobs
...
LIMIT $2`, [MAX_RETRIES, BATCH_SIZE]);
                ...
                raw = await query(`SELECT id, address, 0 AS geocoding_retries
FROM jobs
WHERE geocoding_status = 'pending'
	OR geocoding_status = 'failed'
ORDER BY created_at ASC
LIMIT $1`, [BATCH_SIZE]);
    ...
    async logJobsSchemaInfo() {
        try {
            const rawColumns = await query("SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'jobs' ORDER BY ordinal_position", []);
            const columns = Array.isArray(rawColumns)
                ? rawColumns
                : [];
            const rawDbInfo = await query("SELECT current_database() AS database, current_schema() AS schema", []);
            const dbInfoRows = Array.isArray(rawDbInfo)
                ? rawDbInfo
                : [];
...
SET geocoding_status = 'failed',
	geocoding_retries = $1,
	updated_at = NOW()
WHERE id = $2`, [currentRetries + 1, jobId]);
                }
                catch (err) {
