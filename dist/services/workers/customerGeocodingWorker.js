...
// Customer geocoding
// ============================================================
async function processCustomers(batchSize = 50) {
    const sql = getSql();
...
// Location geocoding
// ============================================================
async function processLocations(batchSize = 50) {
    const sql = getSql();
    const pending = (await sql `
		SELECT id, address, city, state, zip
...
export async function runCustomerGeocodingWorker() {
    ...
    if (customersProcessed > 0 || locationsProcessed > 0) {
        console.log(`📍 Geocoded ${customersProcessed} customers, ${locationsProcessed} locations`);
    }
}
// Retry failed geocoding — run this less frequently (e.g. daily)
// in case the geocoding API was temporarily down
export async function retryFailedGeocoding() {
    const sql = getSql();
