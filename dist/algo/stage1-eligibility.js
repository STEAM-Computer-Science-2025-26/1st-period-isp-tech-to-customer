...
export function checkEligibility(tech, job) {
    const failedRules = [];
    const passedRules = [];
    if (!tech.isActive) {
    ...
    const techCoords = tech.latitude !== null && tech.longitude !== null
    ...
    if (areValidCoordinates(techCoords)) {
        const actualDistance = calculateDistance(techCoords, jobCoords);
        if (actualDistance > tech.maxTravelDistanceMiles) {
            failedRules.push(`Rule 6: Technician is too far away (actual: ${actualDistance.toFixed(2)} mi)`);
        }
        else {
    ...
    const missingSkills = job.requiredSkills.filter((requiredSkill) => {
        ...
        return (!tech.skills.includes(requiredSkill) || techLevel < job.minimumSkillLevel);
    });
    if (missingSkills.length > 0) {
        failedRules.push(`Rule 7: Technician does not meet skill requirements. Missing or insufficient skills: ${missingSkills.join(", ")}`);
    }
    else {
...
export function filterEligibleTechnicians(technicians, job) {
    const eligible = [];
    const ineligible = [];
