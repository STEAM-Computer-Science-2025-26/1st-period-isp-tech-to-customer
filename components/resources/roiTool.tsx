...
export default function RoiTool({ email: _ }: { email: string }) {
	...
	const agmGapClose = Math.max(0, 0.3 - agmPct / 100) * 0.25; // recover 25% of gap
	...
	const paybackDays =
		totalMonthly > 0 ? Math.round(monthlyTTC / (totalMonthly / 30)) : 0;

	const gains = [
		{
			label: "Dispatch Efficiency",
			val: dispatchGain,
			desc: "+12% job throughput from skill-matched routing",
			color: "#53abb1"
		},
		{
			label: "Callback Reduction",
			val: cbSaved,
			desc: "35% fewer callbacks at ~$180 cost each",
			color: "#ffa200"
		},
		{
			label: "Agreement Recovery",
			val: agmRevGain,
			desc: "Closing 25% of your gap to 30% benchmark",
			color: "#52b788"
		},
		{
			label: "Utilization Uplift",
			val: utilGain,
			desc: `Improving from ${utilPct}% toward 80% team utilization`,
			color: "#a8dadc"
		}
	];
		...
		<div className="max-w-[900px] mx-auto px-6 py-10 pb-16">
			<div className="mb-8">
				...
				<h1 className="text-3xl font-bold text-foreground tracking-tight leading-tight mb-2">
					What Does TTC Actually
					<br />
					<span className="text-accent-main">Return on Your Investment?</span>
				</h1>
				<p className="text-[14px] text-text-secondary max-w-lg leading-relaxed">
					No fluff. Enter your real numbers and see what the platform pays back
					across dispatch, callbacks, agreements, and utilization.
				</p>
			</div>
			...
			<div className="grid grid-cols-3 gap-3 mb-3">
				<Slider
					label="Trucks / Techs"
					question="How many trucks are you running?"
					value={trucks}
					min={1}
					max={30}
					step={1}
				...
				<Slider
					label="Monthly Jobs"
					question="Total jobs completed per month"
					value={jobs}
					min={10}
					max={1000}
					step={10}
					onChange={setJobs}
					display={`${jobs.toLocaleString()} jobs`}
				/>
				<Slider
			...
			<div className="grid grid-cols-3 gap-3 mb-5">
				...
				/>
				<Slider
					label="Agreement Penetration"
					question="% of customers with active agreements"
					sub="Benchmark: 25–40%"
					value={agmPct}
					min={0}
					max={60}
				...
				<Slider
					...
					question="% of possible job slots actually filled"
					sub="Benchmark: 75–80%"
					value={utilPct}
					min={30}
					max={100}
			...
			<div className="bg-foreground rounded-xl p-6 mb-3">
				...
				<div className="grid grid-cols-2 gap-8 mb-5">
					<div>
						<div className="font-mono text-[10px] tracking-widest uppercase text-white/30 mb-2">
							Monthly Return
						</div>
						<div className="text-[52px] font-bold text-success-background tracking-tight leading-none mb-1">
							{fmt(totalMonthly)}
						</div>
						<div className="text-[12px] text-white/30">
							vs {fmt(monthlyTTC)}/mo TTC cost
						</div>
					</div>
					<div>
						<div className="font-mono text-[10px] tracking-widest uppercase text-white/30 mb-2">
						...
						<div className="text-[12px] text-white/30">
							net {fmt(netAnnual)} after TTC cost
						</div>
					</div>
				</div>
					...
					{[
						{
							...
							val: `${roiX}×`,
							sub: "return on TTC spend",
							color: "text-accent-main"
						},
						{
							label: "Payback Period",
							val: `${paybackDays} days`,
							sub: "until TTC pays for itself",
						...
						<div key={k.label}>
							...
							<div
								className={`text-[24px] font-bold tracking-tight ${k.color}`}
							>
								{k.val}
							</div>
			...
			>
				<div>
					...
					<p className="text-[13px] text-white/40 max-w-sm leading-relaxed">
						Most TTC customers see results faster than this model projects.
						Start free and track your actual ROI from day one.
					</p>
				</div>
