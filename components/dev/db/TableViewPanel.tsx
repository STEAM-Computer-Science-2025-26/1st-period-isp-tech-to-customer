...
function parseByType(
	...
	if (
		["float", "double", "numeric", "decimal", "real"].some((k) => t.includes(k))
	) {
		const n = Number(raw);
	...
	if (t.includes("bool")) {
		if (["true", "1", "t", "yes"].includes(raw.toLowerCase()))
			return { ok: true, value: true };
		if (["false", "0", "f", "no"].includes(raw.toLowerCase()))
...
export function TableViewPanel(props: {
	...
	const [editingRowDraft, setEditingRowDraft] = useState<Record<
		string,
		string
	> | null>(null);
	...
	const visibleColumns = useMemo(
		() => columns.filter((c) => !hiddenColumns[c.name]),
	...
	function beginResize(columnName: string, startX: number) {
		const startWidth =
			columnWidths[columnName] ?? Math.max(MIN_COL_PX, MIN_COL_PX_RESIZE);
	...
	async function savePendingRow() {
		...
		const { values, missing, invalid } = buildValuesFromDraft(
			pendingRow,
			"insert"
		);
		if (missing.length) {
	...
	return (
		<div className="mt-3">
			<div className="flex items-center gap-2 text-sm">
				...
				{selectedTable ? (
					<span className="text-xs opacity-70">{selectedTable}</span>
				) : null}
			</div>
										...
										<tr className="border-b border-accent-text/30 bg-background-secondary/30">
											...
											<td className="px-2 py-1">
												<label className="text-xs inline-flex items-center gap-2">
													<input
														...
														onChange={(e) =>
															setColumnDraft((p) =>
																p ? { ...p, nullable: e.target.checked } : p
															)
														}
													/>
													Nullable
											...
											<td className="px-2 py-1">
												<input
													...
													onChange={(e) =>
														setColumnDraft((p) =>
															p ? { ...p, defaultValue: e.target.value } : p
														)
													}
												/>
											</td>
					...
					<div className="mt-3 rounded-lg border border-accent-text p-3">
						...
						{!canEditRows ? (
							<div className="mt-2 text-xs opacity-70">
								Row editing is disabled (table has no primary key).
							</div>
						) : null}
